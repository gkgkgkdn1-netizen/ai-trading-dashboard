...
def run_auto_bot():
    print(f"[{datetime.now()}] 🤖 깃허브 자동 브리핑 엔진 가동 시작...")
    if not client:
    ...
    try:
        ...
        # 텔레그램 지능형 묶음 발송 로직
        msg_3_part1 = f"⏰ [정각 자동 브리핑 1/2] 거시/지정학/스캘핑\n\n[거시경제 전문가]\n{all_opinions_dict.get('거시경제 전문가', '')}\n\n[지정학 리스크 전문가]\n{all_opinions_dict.get('지정학 리스크 전문가', '')}\n\n[스캘핑 전문가]\n{all_opinions_dict.get('스캘핑 전문가', '')}"
        msg_3_part2 = f"⏰ [정각 자동 브리핑 2/2] 단타/스윙/추세\n\n[단타 전문가]\n{all_opinions_dict.get('단타 전문가', '')}\n\n[단기 스윙 전문가]\n{all_opinions_dict.get('단기 스윙 전문가', '')}\n\n[추세매매 전문가]\n{all_opinions_dict.get('추세매매 전문가', '')}"
        
        msg_2_part1 = f"⏰ [자동 브리핑 1/3] 거시 & 지정학\n\n[거시경제 전문가]\n{all_opinions_dict.get('거시경제 전문가', '')}\n\n[지정학 리스크 전문가]\n{all_opinions_dict.get('지정학 리스크 전문가', '')}"
        msg_2_part2 = f"⏰ [자동 브리핑 2/3] 스캘핑 & 단타\n\n[스캘핑 전문가]\n{all_opinions_dict.get('스캘핑 전문가', '')}\n\n[단타 전문가]\n{all_opinions_dict.get('단타 전문가', '')}"
        msg_2_part3 = f"⏰ [자동 브리핑 3/3] 스윙 & 추세\n\n[단기 스윙 전문가]\n{all_opinions_dict.get('단기 스윙 전문가', '')}\n\n[추세매매 전문가]\n{all_opinions_dict.get('추세매매 전문가', '')}"

        if len(msg_3_part1) < 4000 and len(msg_3_part2) < 4000:
            send_telegram_message(msg_3_part1)
            time.sleep(1)
            send_telegram_message(msg_3_part2)
        else:
            send_telegram_message(msg_2_part1)
            time.sleep(1)
            send_telegram_message(msg_2_part2)
            time.sleep(1)
            send_telegram_message(msg_2_part3)

        tele_msg_scap = f"🔥 [자동 오더: 단타/스캘핑 팀장] 🔥\n\n현재 BTC 가격: {current_price} USDT\n\n{scap_order}"
        tele_msg_trend = f"📈 [자동 오더: 단기 스윙 팀장] 📈\n\n현재 BTC 가격: {current_price} USDT\n\n{trend_order}"
